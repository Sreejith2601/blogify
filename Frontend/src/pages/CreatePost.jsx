...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(formData.isPublished ? 'Post published!' : 'Draft saved!');
      navigate(result.post?._id ? `/post/${result.post._id}` : '/');
    } catch (err) {
      console.error(err);
    ...
    <div className="bg-brand-bg min-h-screen py-16 sm:py-24 px-4 sm:px-6 md:px-10">
      <div className="max-w-4xl mx-auto">
        <header className="mb-16">
          <h1 className="text-4xl sm:text-6xl font-black text-brand-primary tracking-tighter mb-3">New Story.</h1>
          <p className="text-[10px] font-black text-brand-muted uppercase tracking-[0.2em]">Broadcast your perspective to the global network.</p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="flex items-center justify-between py-12 border-y border-brand-border">
            ...
            <div className="flex items-center gap-8">
              ...
              >
                {loading ? 'Processing...' : formData.isPublished ? 'Broadcast Now' : 'Store Draft'}
              </button>
            </div>
          </div>
        </form>
